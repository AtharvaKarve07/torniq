...
export default function DashboardPage() {
  ...
  const networthHistory = useMemo(() => {
    ...
    return generateMockPriceHistory(base, 14).map((p, i) => ({
      day: `D${i + 1}`,
      value: p.price,
    }));
  }, [stats?.networth]);
      ...
      <AppLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          <div className="p-6 rounded-xl bg-rose-500/10 border border-rose-500/20 max-w-md text-center">
            ...
            <button
              onClick={() => refetch()}
              className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-semibold text-sm rounded-lg transition-colors"
            >
              Try Again
            </button>
          </div>
    ...
    <AppLayout>
      <div className="space-y-6 max-w-7xl">
              ...
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                ...
                {stats?.faction && (
                  <>
                    ...
                    <p className="text-sm text-zinc-400">{stats.faction}</p>
                  </>
                )}
                {stats?.status && (
                  <Badge variant={statusVariant as 'online' | 'offline' | 'hospital'}>
                    {stats.status}
                  </Badge>
                )}
              </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            title="Net Worth"
            value={isLoading ? '—' : formatCash(stats?.networth ?? 0)}
            icon={DollarSign}
            color="amber"
          ...
          <StatCard
            title="Attacks Won"
            value={isLoading ? '—' : formatNumber(stats?.attacks.won ?? 0)}
            icon={Target}
            color="emerald"
          ...
          <StatCard
            ...
            icon={Zap}
            color="blue"
            subtitle="Active trade deals"
          />
          <StatCard
            title="Recent Profits"
            value={isLoading ? '—' : formatCash(stats?.recentProfits ?? 0)}
            icon={TrendingUp}
            color="rose"
          ...
          <SectionCard title="Player Status" description="Current resource levels">
            <div className="space-y-5">
              {isLoading ? (
                ...
                <>
                  <ProgressBar label="Energy" value={stats?.energyPercent ?? 0} color="amber" sublabel={`${stats?.energyPercent ?? 0}%`} />
                  <ProgressBar label="Nerve" value={stats?.nervePercent ?? 0} color="rose" sublabel={`${stats?.nervePercent ?? 0}%`} />
                  <ProgressBar label="Happy" value={stats?.happyPercent ?? 0} color="emerald" sublabel={`${stats?.happyPercent ?? 0}%`} />
                </>
              )}
            </div>
          </SectionCard>
            ...
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={networthHistory}>
                  ...
                  <YAxis tick={{ fill: '#71717a', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={(v) => formatCash(v)} width={65} />
                  <Tooltip
                    formatter={(v: number) => [formatCash(v), 'Net Worth']}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <SectionCard title="Attack Summary">
            <div className="space-y-1">
              {[
                { label: 'Attacks Won', value: formatNumber(stats?.attacks.won ?? 0), color: 'text-emerald-400' },
                { label: 'Attacks Lost', value: formatNumber(stats?.attacks.lost ?? 0), color: 'text-rose-400' },
                ...
                <div key={label} className="flex items-center justify-between py-3 border-b border-zinc-800 last:border-0">
                  ...
                  <span className={`font-syne font-bold tabular-nums ${color}`}>
                    {isLoading ? '—' : value}
                  </span>
                </div>
              ))}
